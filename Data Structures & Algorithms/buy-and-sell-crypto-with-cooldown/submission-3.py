class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        for i in range(len(prices) - 1, -1, -1):
            # buy
            buy = dp[i + 1][0] - prices[i] if i < len(prices) - 1 else -prices[i]
            wait = dp[i + 1][1] if i < len(prices) - 1 else 0
            dp[i][1] = max(buy, wait)

            # sell
            sell = dp[i + 2][1] + prices[i] if i < len(prices) - 2 else prices[i]
            wait = dp[i + 1][0] if i < len(prices) - 1 else 0
